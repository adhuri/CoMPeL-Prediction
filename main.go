...
func main() {
	...
	predictionFrequency := time.Second * 1 // in seconds
		...
		case <-predictionTimer:
			{
				...
				for _, agent := range ContainerInfo {
					...
					for _, containerId := range agent.Containers {
						...
						fmt.Println("For Agent:Container ", agent.ClientIp, ":", containerId)
				...
				/*
					...
					//for every container for CPU
					{
						...
						fetchedDataCPUArray, alignedTimestamp := DataFetcher.GetMetricDataForContainer(TestIp, TestContainer, "cpu", time.Now().Unix(), SlidingWindowSize)
						...
						err1 := DataFetcher.SavePredictedData(TestIp, TestContainer, "cpu_haar_P1", predictedArrayCPUHaar, alignedTimestamp)
						if err1 != nil {
						...
						err3 := DataFetcher.SavePredictedData(TestIp, TestContainer, "cpu_haar_P1_goup", predictedArrayCPUHaarGoUp, alignedTimestamp)
						if err3 != nil {
							fmt.Println("ERROR: Could not store predicted data using SavePredictedData")
						}
						...
						err2 := DataFetcher.SavePredictedData(TestIp, TestContainer, "cpu_max", predictedArrayCPUMax, alignedTimestamp)
						if err2 != nil {
							fmt.Println("ERROR: Could not store predicted data using SavePredictedData")
						}
					...
					{
						//agentIp string, containerId string, metricType string, time int64, numberOfPoints int) returns fetched array and time int64
						fetchedDataMemoryArray, alignedTimestamp := DataFetcher.GetMetricDataForContainer(TestIp, TestContainer, "memory", time.Now().Unix(), SlidingWindowSize)
						...
						err1 := DataFetcher.SavePredictedData(TestIp, TestContainer, "memory_haar", predictedArrayMemoryHaar, alignedTimestamp)
						if err1 != nil {
							fmt.Println("ERROR: Could not store predicted data using SavePredictedData")
						}
						...
						err3 := DataFetcher.SavePredictedData(TestIp, TestContainer, "memory_haar_goup", predictedArrayMemoryHaarGoUp, alignedTimestamp)
						if err3 != nil {
							fmt.Println("ERROR: Could not store predicted data using SavePredictedData")
						}
						...
						err2 := DataFetcher.SavePredictedData(TestIp, TestContainer, "memory_max", predictedArrayMemoryMax, alignedTimestamp)
						if err2 != nil {
							fmt.Println("ERROR: Could not store predicted data using SavePredictedData")
						}
